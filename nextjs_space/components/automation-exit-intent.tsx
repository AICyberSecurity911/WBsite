    ...
    <Dialog open={shouldShow} onOpenChange={closeModal}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden bg-gradient-to-br from-teal-50 via-white to-emerald-50 dark:from-teal-950/30 dark:via-zinc-900 dark:to-emerald-950/30 border-2 border-teal-300 dark:border-teal-700">
        {!submitted ? (
          <>
            {/* Header */}
            <div className="relative bg-gradient-to-r from-teal-600 to-emerald-600 p-6 text-qgd-fg">
              <button
                onClick={closeModal}
                className="absolute top-4 right-4 text-qgd-fg/80 hover:text-qgd-fg transition-colors"
                aria-label="Close"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 bg-qgd-card/20 rounded-xl flex items-center justify-center backdrop-blur-sm">
                  <Download className="w-6 h-6" />
                </div>
                <div>
                  <h2 className="text-2xl font-bold">Wait! Before You Go...</h2>
                  <p className="text-qgd-ring100 text-sm">Get your free automation playbook</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8">
              <div className="mb-6">
                <h3 className="text-xl font-bold text-qgd-fg dark:text-gray-100 mb-3 flex items-center gap-2">
                  <Zap className="w-5 h-5 text-qgd-ring600" />
                  Free Download: 10 Workflows to Automate Computer Use
                </h3>
                <p className="text-qgd-muted dark:text-gray-300 mb-4">
                ...
                {/* What's Inside */}
                <div className="bg-qgd-card dark:bg-zinc-800 rounded-xl p-5 border border-qgd-border dark:border-zinc-700 mb-6">
                  <h4 className="font-semibold text-qgd-fg dark:text-gray-100 mb-3 text-sm">
                    What's Inside:
                  </h4>
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={i} className="flex items-start gap-2 text-sm">
                        <CheckCircle2 className="w-4 h-4 text-qgd-ring600 flex-shrink-0 mt-0.5" />
                        <span className="text-qgd-muted dark:text-gray-300">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-qgd-fg font-semibold py-6 text-lg"
                >
                  {loading ? (
                    'Sending...'
                  ) : (
                ...
                <p className="text-xs text-center text-qgd-muted dark:text-gray-400">
          ...
          <div className="p-8 text-center">
            <div className="w-20 h-20 bg-gradient-to-br from-emerald-400 to-teal-400 rounded-full flex items-center justify-center mx-auto mb-6">
              <Mail className="w-10 h-10 text-qgd-fg" />
            </div>
            
            <h3 className="text-2xl font-bold text-qgd-fg dark:text-gray-100 mb-3">
              Check Your Inbox!
            </h3>
            
            <p className="text-qgd-muted dark:text-gray-300 mb-6">
              Your <strong>10 Workflows to Automate Computer Use</strong> guide is on its way to <strong className="text-qgd-ring600 dark:text-qgd-ring400">{email}</strong>
            </p>
            
            <div className="bg-qgd-primary/50 dark:bg-qgd-primary/900/20 rounded-xl p-4 border border-teal-200 dark:border-teal-800 mb-6">
              <p className="text-sm text-qgd-muted dark:text-gray-300">
            ...
            <Button
              onClick={closeModal}
              variant="outline"
              className="border-2 border-teal-600 text-qgd-ring700 dark:text-qgd-ring400 hover:bg-qgd-primary/50 dark:hover:bg-qgd-primary/950/30"
            >
              Continue Exploring
            </Button>
          </div>
