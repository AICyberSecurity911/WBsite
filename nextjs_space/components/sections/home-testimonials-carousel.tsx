...
export function HomeTestimonialsCarousel() {
  ...
  return (
    <section className="relative py-20 lg:py-28 bg-qgd-bg overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-[url('/grid-pattern.svg')] opacity-5" />
      
      {/* Animated gradient orbs */}
      <div className="absolute top-20 left-10 w-72 h-72 bg-qgd-accent/10 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-20 right-10 w-96 h-96 bg-qgd-primary/10 rounded-full blur-3xl animate-pulse delay-700" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-qgd-accent/20 rounded-full mb-4">
            <Star className="w-5 h-5 text-qgd-accent fill-qgd-accent" />
            <span className="text-sm font-semibold text-qgd-fg">Client Success Stories</span>
          </div>
          
          <h2 className="text-4xl lg:text-5xl font-bold text-qgd-fg leading-tight mb-4">
            Real Results from{' '}
          ...
          <p className="text-lg text-qgd-muted max-w-2xl mx-auto">
        ...
        >
          ...
          >
            {testimonials.map((testimonial, index) => (
              <SwiperSlide key={index}>
                <motion.div 
                  className="bg-qgd-card/40 backdrop-blur-sm rounded-2xl shadow-2xl p-8 md:p-12 border border-qgd-border hover:border-qgd-accent hover:shadow-qgd transition-all duration-500"
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ duration: 0.6, ease: "easeOut" }}
                >
                  {/* Category Badge */}
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-qgd-accent/20 rounded-full mb-6">
                    <TrendingUp className="w-4 h-4 text-qgd-accent" />
                    <span className="text-xs font-semibold text-qgd-accent">{testimonial.category}</span>
                  </div>

                  {/* Quote */}
                  <div className="relative mb-8">
                    <Quote className="absolute -top-2 -left-2 w-12 h-12 text-qgd-accent/20" />
                    <blockquote className="relative text-xl md:text-2xl font-medium text-qgd-fg leading-relaxed pl-8">
                      "{testimonial.quote}"
                    </blockquote>
                  </div>

                  {/* Before/After */}
                  <div className="grid md:grid-cols-2 gap-6 mb-8 p-6 bg-qgd-bg/60 rounded-xl border border-qgd-border">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-qgd-primary" />
                        <span className="text-sm font-bold text-qgd-fg uppercase tracking-wider">Before</span>
                      </div>
                      <p className="text-sm text-qgd-muted leading-relaxed">{testimonial.before}</p>
                    </div>
                    
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-qgd-accent" />
                        <span className="text-sm font-bold text-qgd-fg uppercase tracking-wider">After</span>
                      </div>
                      <p className="text-sm text-qgd-muted leading-relaxed">{testimonial.after}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    <div>
                      <div className="font-bold text-lg text-qgd-fg">
                        {testimonial.author}
                      </div>
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: testimonial.rating }).map((_, i) => (
                        <Star key={i} className="w-5 h-5 text-qgd-accent fill-qgd-accent" />
                      ))}
                    </div>
                  </div>
                </motion.div>
          ...
          <button 
            className="testimonial-prev absolute left-0 top-1/2 -translate-y-1/2 -translate-x-12 z-10 w-12 h-12 rounded-full bg-qgd-card/80 backdrop-blur-sm shadow-lg flex items-center justify-center text-qgd-fg hover:bg-qgd-accent hover:text-qgd-bg hover:scale-110 transition-all duration-500 disabled:opacity-50 disabled:cursor-not-allowed border border-qgd-border"
          ...
          <button 
            className="testimonial-next absolute right-0 top-1/2 -translate-y-1/2 translate-x-12 z-10 w-12 h-12 rounded-full bg-qgd-card/80 backdrop-blur-sm shadow-lg flex items-center justify-center text-qgd-fg hover:bg-qgd-accent hover:text-qgd-bg hover:scale-110 transition-all duration-500 disabled:opacity-50 disabled:cursor-not-allowed border border-qgd-border"
        ...
        >
          <a 
            href="/consultation"
            className="inline-flex items-center gap-2 px-8 py-4 bg-qgd-primary hover:bg-qgd-accent text-qgd-bg font-semibold rounded-xl shadow-lg hover:shadow-qgd hover:scale-105 transition-all duration-500 group"
          >
            Get Your Free Assessment
            <ArrowUpRight className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" />
          </a>
          <p className="text-sm text-qgd-muted mt-3">
