...
interface FlameBorderProps {
  ...
  color?: string;   // Flame color
  seg?: number;     // Segment length (1-1000)
  width?: number;   // Stroke width in pixels
  dur?: number;     // Duration in seconds per lap
  className?: string;
}

/**
 * FlameBorder - Animated copper flame traveling around card borders
 * Part of Quantum Gradient (Dark) design system
 */
export const FlameBorder: React.FC<FlameBorderProps> = ({
  ...
  return (
    <svg
      className={`pointer-events-none absolute inset-0 z-10 mix-blend-screen h-[calc(100%+12px)] w-[calc(100%+12px)] translate-x-[-6px] translate-y-[-6px] ${className}`}
      viewBox="0 0 100 100"
